...
def _cf_agent_headers() -> dict[str, str]:
    """Auth header sent to every Cloudflare agent Worker. Must match the Worker's
    AGENT_API_KEY secret; the Worker fail-closes (503) if its secret is unset."""
    key = settings.cf_agent_api_key
    ...
    async def run(self):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                response = await client.post(worker_url, json=payload, headers=_cf_agent_headers())
...
class CloudflareProxmoxAgentRunner:
    ...
    async def run(self):
        ...
        payload = {
            ...
            "allow_destructive": settings.cf_proxmox_allow_destructive,
        }
