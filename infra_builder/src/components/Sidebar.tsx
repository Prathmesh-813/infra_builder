...
import { Zap, Search, X, AlertTriangle, FileCode2, Settings, Layers3, BarChart3, GripVertical, ChevronRight } from 'lucide-react';
...
// Fresh, vibrant palette used to colour-code the collapsible category groups.
const CAT_PALETTE = ['#8b5cf6', '#06b6d4', '#10b981', '#f59e0b', '#ef4444', '#ec4899', '#3b82f6', '#14b8a6', '#a855f7', '#f97316', '#eab308', '#22d3ee'];
...
export default function Sidebar({ onDragStart, onOpenBlueprints, fixedMode }: SidebarProps) {
  ...
  // ── Collapsible, colour-coded category groups ───────────────────────────────
  const grouped = useMemo(() => {
    const m: Record<string, ResourceDefinition[]> = {};
    for (const d of activeDefs) (m[d.category] ||= []).push(d);
    return m;
  }, [activeDefs]);
  const [collapsedCats, setCollapsedCats] = useState<Set<string>>(new Set());
  const toggleCat = (c: string) => setCollapsedCats((prev) => {
    const n = new Set(prev); n.has(c) ? n.delete(c) : n.add(c); return n;
  });
  const searching = search.trim().length > 0;
  const matchDef = (d: ResourceDefinition) => {
    const s = search.trim().toLowerCase();
    return !s || d.label.toLowerCase().includes(s) || d.type.toLowerCase().includes(s);
  };
  ...
  // Drag a resource onto the canvas with a styled drag-preview chip (instead of
  // the browser's default washed-out element snapshot).
  const handleCardDragStart = (e: React.DragEvent, def: ResourceDefinition) => {
    const ghost = document.createElement('div');
    ghost.textContent = `${def.icon}  ${def.label}`;
    ghost.style.cssText =
      'position:fixed;top:-1000px;left:-1000px;padding:9px 16px;border-radius:12px;' +
      'font:600 13px Inter,system-ui,sans-serif;color:#e2e8f0;white-space:nowrap;' +
      'background:linear-gradient(135deg,#1e293b,#0f172a);' +
      'border:1px solid rgba(99,102,241,0.55);box-shadow:0 14px 44px rgba(99,102,241,0.45);';
    document.body.appendChild(ghost);
    try { e.dataTransfer.setDragImage(ghost, 24, 22); } catch { /* ignore */ }
    setTimeout(() => { try { document.body.removeChild(ghost); } catch { /* ignore */ } }, 0);
    onDragStart(e, def);
  };
  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-2.5">
        <div className="relative">
          ...
          <input
            ...
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full text-[11px] pl-8 pr-8 py-2 rounded-lg transition-all outline-none focus:shadow-[0_0_0_3px_rgba(99,102,241,0.18)]"
      ...
      {/* Palette header: live count + expand/collapse all */}
      <div className="flex items-center justify-between px-3 py-1.5 border-b" style={{ borderColor: 'var(--border-subtle)' }}>
        ...
        <button
          onClick={() => setCollapsedCats(collapsedCats.size ? new Set() : new Set(activeCategories))}
          className="text-[9px] font-semibold transition-colors hover:underline"
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 gap-2">
            ...
            <p className="text-[11px]" style={{ color: 'var(--text-muted)' }}>No resources found</p>
          </div>
        ) : activeCategories.map((cat, ci) => {
          const defs = (grouped[cat] ?? []).filter(matchDef);
          if (defs.length === 0) return null;
          const color = CAT_PALETTE[ci % CAT_PALETTE.length];
          const isCollapsed = !searching && collapsedCats.has(cat);
          return (
            <div key={cat} className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--border-subtle)' }}>
              {/* Category header */}
              <button
                onClick={() => toggleCat(cat)}
                className="w-full flex items-center gap-2 px-2.5 py-2 transition-all hover:brightness-125"
                style={{ background: `linear-gradient(90deg, ${hexA(color, 0.13)}, transparent)`, borderLeft: `2px solid ${color}` }}
              >
                <ChevronRight size={12} className="shrink-0 transition-transform duration-200" style={{ color, transform: isCollapsed ? 'none' : 'rotate(90deg)' }} />
                <span className="text-sm leading-none">{CAT_ICONS[cat] ?? '▸'}</span>
                <span className="flex-1 text-left text-[10px] font-bold uppercase tracking-wider truncate" style={{ color: 'var(--text-secondary)' }}>{cat}</span>
                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full" style={{ background: hexA(color, 0.18), color }}>{defs.length}</span>
              </button>

              {/* Cards */}
              {!isCollapsed && (
                <div className="p-1.5 space-y-1" style={{ background: 'rgba(0,0,0,0.12)' }}>
                  {defs.map((def) => (
                    <div
                      key={def.type + def.label}
                      draggable
                      onDragStart={(e) => handleCardDragStart(e, def)}
                      className={`group resource-card ${def.bgColor} border rounded-xl p-2.5 cursor-grab active:cursor-grabbing select-none`}
                      style={{ borderColor: 'rgba(255,255,255,0.08)' }}
                      title={`${def.description}\n\nDrag onto the canvas`}
                    >
                      <div className="flex items-center gap-2.5">
                        <div
                          className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-lg leading-none transition-transform group-hover:scale-110"
                          style={{ background: hexA(color, 0.16), border: `1px solid ${hexA(color, 0.32)}` }}
                        >
                          {def.icon}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className={`font-semibold text-[11px] ${def.color} leading-tight truncate`}>{def.label}</div>
                          <div className="text-[9px] mt-0.5 truncate font-mono" style={{ color: 'var(--text-muted)' }}>{def.type}</div>
                        </div>
                        {def.ports.length > 0 && (
                          <div className="flex-shrink-0 text-[8px] font-bold px-1.5 py-0.5 rounded-full transition-opacity group-hover:opacity-0"
                            style={{ background: 'rgba(0,0,0,0.3)', color: '#64748b', border: '1px solid rgba(255,255,255,0.07)' }}>
                            {def.ports.length}p
                          </div>
                        )}
                        <GripVertical size={14} className="flex-shrink-0 -ml-1 opacity-0 group-hover:opacity-60 transition-opacity" style={{ color: 'var(--text-muted)' }} />
                      </div>
                    </div>
                  ))}
                </div>
