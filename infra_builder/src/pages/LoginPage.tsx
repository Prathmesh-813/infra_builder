...
import { Cpu, Eye, EyeOff, Mail, Lock, UserPlus, LogIn, ArrowRight, AlertCircle, Zap, KeyRound, CheckCircle2 } from 'lucide-react';
import { ozLogin, ozRegister, ozResetPassword, setToken } from '../utils/ozClient';
...
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signup') {
        await ozRegister(email, password, name || email);
        localStorage.setItem('infrastudio_logged_in', 'true');
        onLogin();
      } else if (mode === 'forgot') {
        await ozResetPassword(email, password);
        setSubmitted(false);
        setResetDone(true);
      } else {
  ...
  }, [mode, email, password, confirmPassword, name, onLogin]);
    ...
    <div className="h-screen w-screen flex overflow-hidden"
      ...
      <div className="relative z-10 m-auto w-full max-w-md px-4">
        <div className={`rounded-2xl overflow-hidden shadow-2xl ${submitted ? 'opacity-0 scale-95 transition-all duration-500' : 'login-slide-up'}`}
          ...
          {mode === 'forgot' && (
            <div className="mx-8 mb-6 login-fade-in">
              <div className="flex items-center gap-2 mb-1">
                <KeyRound size={15} style={{ color: 'var(--accent-light)' }} />
                <span className="text-sm font-bold" style={{ color: 'var(--text-primary)' }}>Reset Password</span>
              </div>
          ...
          <form onSubmit={handleSubmit} className="px-8 pb-8 space-y-4">
            ...
            {resetDone && (
              <div className="flex flex-col items-center gap-3 py-4 login-fade-in">
                <CheckCircle2 size={36} style={{ color: '#22c55e' }} />
                <p className="text-sm font-bold text-center" style={{ color: 'var(--text-primary)' }}>Password updated!</p>
                <p className="text-[11px] text-center" style={{ color: 'var(--text-faint)' }}>
                  You can now sign in with your new password.
                </p>
                <button type="button" onClick={goToSignIn}
                  className="mt-1 flex items-center gap-1.5 text-xs font-semibold hover:underline"
            ...
            {!resetDone && (
              <>
                ...
                {error && (
                  <div className="flex items-center gap-2 text-[11px] rounded-lg px-3 py-2 login-fade-in"
                ...
                <button type="submit"
                  ...
                  <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                    style={{ background: mode === 'forgot' ? 'linear-gradient(135deg, #fb923c, #fdba74)' : 'linear-gradient(135deg, #818cf8, #a78bfa)' }} />
                  <span className="relative flex items-center gap-2">
                    {mode === 'signin'  && <><LogIn size={15} /> Sign In</>}
                    {mode === 'signup'  && <><UserPlus size={15} /> Create Account</>}
                    {mode === 'forgot'  && <><KeyRound size={15} /> Reset Password</>}
            ...
            {mode !== 'forgot' && (
              <>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-semibold transition-all duration-200 hover:scale-[1.01]"
